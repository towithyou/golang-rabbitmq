...
func main() {
	...
	err = channel.Publish(exchangeName, routingKeyError, true, false, amqp.Publishing{
		Body: []byte("return listener"),
	})
	if err != nil {
		fmt.Println(err)
		return
	...
	go func() {
		...
		for r := range c {
			fmt.Println(r.ReplyCode, r.ReplyText, r.Exchange, string(r.Body))
		}
